...
pub fn number(input: &str) -> ParseResult<&str, i32> {
    map_res(recognize(tuple((opt(tag_custom("-")), digit1))), str::parse)(input)
}
...
pub fn abs_tag(input: &str) -> ParseResult<&str, EV_ABS> {
    alt((
        ...
        alt((map(tag_no_case("MT_TOOL_Y"), |_| ABS_MT_TOOL_Y), map(tag_no_case("MAX"), |_| ABS_MAX))),
    ))(input)
}

pub fn motion_action(input: &str) -> ParseResult<&str, KeyAction> {
    map_res(tuple((alt((tuple((ident, multispace1, ident)),)), multispace1, number)), |((tag1, _, tag2), _, value)| {
        let event_code = match &*tag1 {
            "relative" => match &*tag2.to_uppercase() {
            ...
            "absolute" => match &*tag2.to_uppercase() {
...
mod tests {
    ...
    fn motion_action_input() {
        assert_eq!(
            motion_action("relative X 33"),
            nom_ok(KeyAction { key: Key { event_code: EventCode::EV_REL(REL_X) }, value: 33 })
        );

        assert_eq!(
            motion_action("relative Y 99"),
            nom_ok(KeyAction { key: Key { event_code: EventCode::EV_REL(REL_Y) }, value: 99 })
        );

        assert_eq!(
            motion_action("absolute Z 99"),
            nom_ok(KeyAction { key: Key { event_code: EventCode::EV_ABS(ABS_Z) }, value: 99 })
        );

        assert_eq!(
            motion_action("absolute hat0x 1"),
            nom_ok(KeyAction { key: Key { event_code: EventCode::EV_ABS(ABS_HAT0X) }, value: 1 })
        );

        assert_eq!(
            motion_action("absolute TILT_X -5"),
            nom_ok(KeyAction { key: Key { event_code: EventCode::EV_ABS(ABS_TILT_X) }, value: -5 })
        );
    }
