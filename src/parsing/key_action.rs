...
impl ParsedKeyActionVecExt for Vec<ParsedKeyAction> {
    ...
    fn to_input_ev(self) -> Vec<EvdevInputEvent> {
        self.to_key_actions().into_iter().map(|x| x.to_input_ev()).collect()
    }
}
...
pub fn complex_key_action_utf<'a>(
    ...
    move |input: &str| {
        alt((
            ...
            map(tuple((key_utf(transformer), multispace1, action_state)), |((key, mods), _, state)| {
                ((key, Some(mods)), Some(state))
            }),
            // motion action - relative X 20
            map(motion_action, |action| ((action.key, None), Some(action.value))),
        ))(input)
    }
}
        ...
        map_res(
            ...
            |((key, mods), value)| {
                let action = match value {
                    ...
                    None => ParsedKeyAction::KeyClickAction(KeyClickActionWithMods::new_with_mods(
                        key,
    ...
    move |input: &str| {
        map_res(
            alt((
                ...
                map(surrounded_group("{", "}", key_utf(transformer)), |(key, mods)| ((key, Some(mods)), None)),
                // escaped special char - \\{
                map(tuple((tag_custom("\\"), key_utf(transformer))), |(_, (key, mods))| ((key, Some(mods)), None)),
                // any 1 char except special ones
                map_res(recognize(none_of("{}")), |input| {
...
mod tests {
    ...
    fn action_with_state() {
        ...
        assert_eq!(
            key_action("{shift down}"),
            nom_ok(ParsedKeyAction::KeyAction(KeyActionWithMods::new(
                Key::from_str("KEY_LEFTSHIFT").unwrap(),
                1,
                KeyModifierFlags::new()
            )))
        );
        ...
        assert_eq!(
            key_action("{relative X 99}"),
            nom_ok(ParsedKeyAction::Action(KeyAction { key: Key { event_code: EventCode::EV_REL(REL_X) }, value: 99 }))
        );

        assert_eq!(
            key_action("{absolute X 99}"),
            nom_ok(ParsedKeyAction::Action(KeyAction { key: Key { event_code: EventCode::EV_ABS(ABS_X) }, value: 99 }))
        );
    }
    ...
    fn action_with_mods() {
        assert_eq!(
            single_key_action_with_flags("+a down"),
            nom_ok(ParsedKeyAction::KeyAction(KeyActionWithMods::new(
                Key::from_str("KEY_A").unwrap(),
                1,
        ...
        assert_eq!(
            single_key_action_with_flags("!j down"),
            nom_ok(ParsedKeyAction::KeyAction(KeyActionWithMods::new(
    ...
    fn single_key_action_input() {
        ...
        assert_eq!(
            single_key_action_utf_with_flags_utf(Some(&t))("page_down"),
            nom_ok(ParsedKeyAction::KeyClickAction(KeyClickActionWithMods::new_with_mods(
                KEY_PAGEDOWN.into(),
                KeyModifierFlags::new()
            )))
