...
pub fn key_sequence_utf<'a>(
    ...
    move |input: &str| many1_with_last_err(key_action_utf(transformer))(input)
}
...
mod tests {
    ...
    fn sequence_input() {
        ...
        assert_eq!(
            key_sequence("a{b down}c"),
            nom_ok((
                vec![
                    ParsedKeyAction::KeyClickAction(KeyClickActionWithMods::new(KEY_A.into())),
                    ParsedKeyAction::KeyAction(KeyActionWithMods::new(
                        KEY_B.into(),
                        TYPE_DOWN,
        ...
        assert_eq!(
            key_sequence("{shift down}a{shift up}"),
            nom_ok((
                vec![
                    ParsedKeyAction::KeyAction(KeyActionWithMods::new(
                        KEY_LEFTSHIFT.into(),
    ...
    fn sequence_special_chars() {
        assert_eq!(
            key_sequence("{relative X 55}"),
            nom_ok((
                vec![ParsedKeyAction::Action(KeyAction::new(Key { event_code: EventCode::EV_REL(REL_X) }, 55)),],
                CustomError { input: "", expected: vec![] }
            ))
        );
    }
